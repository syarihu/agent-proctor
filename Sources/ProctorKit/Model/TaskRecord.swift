...
public struct TaskRecord: Codable, Equatable {
    ...
    /// 「この作業はこれ」と決めて付けた名前。
    ///
    /// **書き手は2つある。** 人が端末のタブに付けたタイトル (`tab_title`。
    /// hooks が毎イベント運んでくる) と、セッション自身が付けた名前
    /// (`proctor title` = `NameSession.run`)。同じ欄を共有していて、
    /// **後から来たほうが勝つ**。`tab_title` を毎イベント送るフックを書くと、
    /// セッションが付けた名前はその次のイベントで静かに上書きされる。
    ///
    /// エージェントが自分で付ける `name` より、こちらを先に出す
    /// (あちらは会話から勝手に決まるもので、的外れなことがある)
