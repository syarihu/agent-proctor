...
public final class SidebarPanel: NSObject {
    ...
    private func updateLevel() {
        // 浮いている端末 (hotkey window) には、誰が前にいるかに関係なく寄り添う。
        //
        // 高さを「iTerm2 が前面アプリか」で決めていると、ホットキーで呼んだ直後の、
        // 窓はもう出ているのに前面がまだ入れ替わっていない一瞬で下がったままになり、
        // 22 のような高い層の下へ潜ってサイドバーが出てこない。
        // クリックして初めて出てくるのはこのため。
        //
        // 前面を気にしなくてよいのは、浮く hotkey window が自分で引っ込む窓だから。
        // 別のアプリへ移れば端末ごと消えて、こちらも相手を見失って引っ込む。
        // 「別のアプリに移ったから下がる」という気遣いは、居座る窓のためのもの。
        //
        // オフィス窓を先に見ないのも同じ理由。あちらは通常より1段上なだけなので、
        // 浮いている端末より下にいる。サイドバーがその両方の上に立つ順序で正しい
        if itermLayer > NSWindow.Level.normal.rawValue {
            panel.level = alongsideTerminalLevel
            return
        }
