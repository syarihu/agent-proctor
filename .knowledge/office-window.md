...
| 窓 | 層 | 決め方 |
| --- | --- | --- |
| サイドバー (`SidebarPanel`) | 端末の層 + 1 | **読んで決める** |
| iTerm2 hotkey window | iTerm2 が決める (実機で **22**) | 相手任せ |
| オフィス窓 (`OfficeWindow`) | `.normal` + 1 | 決め打ち (唯一) |
| 普通のウィンドウ | `.normal` (0) | |

**前提**: iTerm2 の「Floating window」が入っていること。入っていないと端末が 0 に来て
オフィス窓 (1) の下に潜る。`scripts/setup-iterm-hotkey.py` がこれを入れる。
