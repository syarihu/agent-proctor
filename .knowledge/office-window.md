...
`c67bc86` で前面に出ない窓にしてから `313a073` で戻すまで、13本。
**1つ直すたびに次が出る形になっていて、それ自体が引き返す合図だった。**

| # | やったこと | 出てきた問題 |
| --- | --- | --- |
| 1 | 非アクティブ化パネルにしてステージの入れ替えを止めた | 端末を引っ込めるとオフィス窓も消える (ように見える) |
| 2 | 窓を出したとき前にいたアプリを覚えて例外にした | サイドバーがオフィス窓の上に取り残される |
| 3 | オフィス窓が出ている間サイドバーを下げた | そのあと端末へ移ってもサイドバーが戻らない |
| 4 | 「出ているか」でなく「手前か」を自前で数えた | オフィス窓が相手のウィンドウに埋まる |
| 5 | オフィス窓を1段上げた | 端末がオフィス窓の下に潜る |
| 6 | iTerm2 の hotkey window を浮かせてもらった | サイドバーが端末を見失う (層 0 で絞っていた) |
| 7 | 層で絞らず、読んで +1 に立つようにした | 呼んだ直後はまだ前面が入れ替わっておらず出てこない |
| 8 | 浮いた端末には前面を問わず寄り添うようにした | **浮いた hotkey window は焦点を取らない。端末に打てない** |
| 9 | 浮かせるのをやめ、オフィス窓の層も戻した | 埋まる問題が再発 (5 に戻った) |
| 10 | 相手が落ち着いてからもう一度出し直す (2回叩く) | 一瞬相手が見える。打鍵の宛先にもなっていない |
| 11 | 押したらキーボードを受け取るようにした | ⌘N で端末を開いて閉じると焦点が返らない |
| 12 | **引き返した** (`313a073`) | ステージの入れ替えは受け入れる |

### 引き返すと決めた理由

- **直した数だけ次が出た。** 10 で入れた「2回叩く」は競争に勝つためのタイマーで、
  これは「土俵が違う」と認めたのと同じだった
- **得ているものが1つしかない。** 払っているのは、前面・キーボード・重なり順という
  OS が持っている仕組みを自前で数え直すこと。数え直しはどれも競争か取りこぼしを抱える
- 11 まで来て「窓をクリックしないとキーボードが来ない」という、
  **窓としての当たり前が満たせていない**ことがはっきりした

### 次に同じ判断をするための目印

- **他アプリの設定を要求し始めたら**、それは自分の都合を外へ押し出している (6)
- **順序の競争にタイマーで勝とうとし始めたら**、層や仕組みで決まる話に持ち込めていない (10)
- **OS が答えを持っている問いを自前で数え始めたら** (「いま手前か」「前にいたのは誰か」)、
  その答えを捨てる選択をした帰結が出ている (2・4)
