...
サイドバーを見ていないときは、macOS の通知が代わりに知らせる。確認待ちになった・
終わった・落ちた、その瞬間に1件出す。何の承認を待っているかも本文に載る。
押すと一覧の行と同じくそのタブへ移動する。**いま見ているタブのことは知らせない**
——iTerm2 が前面で、そのタブを開いている間に起きたことはもう画面に出ているので、
その上にバナーを重ねても邪魔になるだけ。起動した瞬間にも出さない。開き直したことは
知らせるほどのことではないし、前から確認待ちのものは自分で置いてきたものになる。
片が付いたもの（承認に答えた、終わったタブを見た）は通知センターからも下ろすので、
済んだ用事で埋まらない。3つはそれぞれ「設定… → 通知」で切れる。出し方
（バナーか通知か、音を鳴らすか）は他のアプリと同じく システム設定 → 通知 の側にある。
...
通知の許可も初回に尋ねてくる。こちらの状態も同じ「許可」の欄に出ていて、
そこからシステム設定の該当ページへ行ける。断って困るのはバナーが出ないことだけで、
サイドバーも `要確認` のストリップも今までどおり動く。
