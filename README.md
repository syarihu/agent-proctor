...
- **Multi-Level Organization**: Sessions are automatically grouped by GitHub repository and organization/owner (with avatar icons). Groups can be folded independently with compact tally summaries (`⏳1 ▶2 ⌁2`). Switch how rows are grouped — by status (waiting, in progress, done), by repository, or by organization — with the tabs at the top of the sidebar.
